...
def gettime():
      timeInterval=int(time.time()-targetTime)#计算时间差
      timestr = time.strftime("%Y-%m-%d %H:%M:%S") # 获取当前的时间并转化为字符串
      lb.configure(text='\n当前时间:'+timestr+'\n\n热调试已连续运行\n\n'+str(int(timeInterval/86400))+'天'+str(int(timeInterval%86400/3600))+'小时'+str(int(timeInterval%3600/60))+'分钟'+str(int(timeInterval%60))+'秒')   # 重新设置标签文本
